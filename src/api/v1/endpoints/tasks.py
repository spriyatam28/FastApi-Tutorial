...
@router.delete("/{user_id}", response_model=TaskResponse, status_code=status.HTTP_200_OK)
async def delete_tasks(user_id: int, db: AsyncSession = DB_SESSION):
	"""
	Deletes all the tasks of a user
	:param user_id: id of a user
	:param db: Database
	:return: An empty `TaskResponse` with `user_id` in it
	"""
	repo = TaskRepository(db)
	service = TaskService(repo)
